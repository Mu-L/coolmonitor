...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log('');
    console.log('示例:');
    console.log('  npm run reset-password admin');
    console.log('  npm run reset-password admin "新密码123"');
    console.log('');
    console.log('如果不提供新密码,将自动生成一个随机密码');
    console.log('在 Docker 中使用:');
    console.log('  docker exec -it coolmonitor npm run reset-password admin');
    console.log('  docker exec -it coolmonitor npm run reset-password admin "新密码123"');
    process.exit(0);
  }
